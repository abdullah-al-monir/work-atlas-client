import { useContext, useEffect, useState } from "react";
import useDocumentTitle from "../../hooks/useTitle";
import axios from "axios";
import { AuthContext } from "../../providers/AuthProvider";
import Swal from "sweetalert2";
import { Link } from "react-router-dom";
import { Spinner } from "@material-tailwind/react";

const MyJobs = () => {
  const { user, setLoading } = useContext(AuthContext);
  const [jobs, setJobs] = useState([]);
  const userId = user.uid;
  const title = "Work Atlas | My Jobs";
  useDocumentTitle(title);
  const [jobLoading, setJobLoading] = useState(true);
  useEffect(() => {
    axios
      .get(`https://work-atlas-server.vercel.app/myJobs?userId=${userId}`, {
        withCredentials: true,
      })
      .then((res) => {
        setJobs(res.data);
        setJobLoading(false);
        setLoading(false);
      });
  }, [userId, setLoading]);
  const handleDeleteJob = (id) => {
    axios
      .delete(`https://work-atlas-server.vercel.app/allJobs/${id}`)
      .then((res) => {
        console.log(res.data);
        if (res.data.deletedCount > 0) {
          Swal.fire(
            "Deleted!",
            "You have deleted the post successfully",
            "success"
          );
          const remainingJobs = jobs.filter((job) => job._id !== id);
          setJobs(remainingJobs);
        }
      });
  };

  return (
    <div>
      {jobLoading ? (
        <div className="flex justify-center items-center h-[100vh]">
          <Spinner className="h-10 w-10 text-center text-primary" />
        </div>
      ) : (
        <div>
          {jobs.length > 0 ? (
            <section className="container px-4 mx-auto mt-10">
              <div className="flex items-center gap-x-3">
                <h2 className="text-xl font-medium text-secondary">
                  Jobs posted by {user.displayName}
                </h2>

                <span className="px-3 py-1 text-xs text-blue-600 bg-blue-100 rounded-full  ">
                  {jobs.length} Jobs
                </span>
              </div>

              <div className="flex flex-col mt-6">
                <div className="-mx-4 -my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
                  <div className="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
                    <div className="overflow-hidden border border-gray-200  md:rounded-lg">
                      <table className="min-w-full divide-y divide-gray-200 ">
                        <thead className="bg-black text-sm md:text-base">
                          <tr>
                            <th
                              scope="col"
                              className="py-3.5 px-4 text-secondary "
                            >
                              <div className="flex items-center gap-x-3">
                                <span>Job</span>
                              </div>
                            </th>

                            <th
                              scope="col"
                              className="px-2 md:px-4 py-3.5  text-secondary text-center"
                            >
                              <button className="">
                                <span>Job Type</span>
                              </button>
                            </th>

                            <th
                              scope="col"
                              className="px-4 py-3.5 text-secondary text-center "
                            >
                              <button className="">
                                <span>Application Deadline</span>
                              </button>
                            </th>
                            <th
                              scope="col"
                              className="relative py-3.5 px-4 text-right text-secondary"
                            >
                              <span>Edit</span>
                            </th>
                          </tr>
                        </thead>
                        <tbody className="bg-white divide-y divide-gray-200  ">
                          {jobs.map((job) => (
                            <tr key={job._id}>
                              <td className="px-4 py-4 text-sm whitespace-nowrap">
                                <div className="inline-flex items-center gap-x-3">
                                  <div className="flex items-center gap-x-2">
                                    <div>
                                      <h2 className="font-medium text-gray-800  text-base">
                                        {job.jobTitle}
                                      </h2>
                                      <div className="flex items-center gap-1">
                                        <span>Company: </span>
                                        <img
                                          className="h-2 md:h-3"
                                          src={job.companyLogo}
                                          alt=""
                                        />
                                      </div>
                                      <p className="text-sm font-normal">
                                        Applicants: {job.NumberOfApplicants}
                                      </p>
                                    </div>
                                  </div>
                                </div>
                              </td>
                              <td className=" md:px-8 py-4 text-xs md:text-sm font-medium text-gray-700 whitespace-nowrap text-center">
                                <button
                                  className="px-3 py-1 text-indigo-500 rounded-full bg-indigo-100/60"
                                  disabled
                                >
                                  {job.category}
                                </button>
                              </td>
                              <td className="md:px-4 py-4 whitespace-nowrap text-xs md:text-sm text-center">
                                <button
                                  className="px-4 py-1 text-red-600 rounded-full bg-red-100/60 "
                                  disabled
                                >
                                  {job.applicationDeadline}
                                </button>
                              </td>
                              <td className="px-4 py-4 text-xs md:text-sm whitespace-nowrap">
                                <div className="flex items-center gap-x-6 justify-end">
                                  <button
                                    onClick={() => handleDeleteJob(job._id)}
                                    className=" transition-colors duration-200 hover:text-red-700 text-red-500 focus:outline-none"
                                  >
                                    <svg
                                      xmlns="http://www.w3.org/2000/svg"
                                      fill="none"
                                      viewBox="0 0 24 24"
                                      strokeWidth="1.5"
                                      stroke="currentColor"
                                      className="w-4 md:w-5 h-4 md:h-5"
                                    >
                                      <path
                                        strokeLinecap="round"
                                        strokeLinejoin="round"
                                        d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0"
                                      />
                                    </svg>
                                  </button>
                                  <Link to={`/update/${job._id}`}>
                                    <button className=" transition-colors duration-200 hover:text-green-700 text-green-500 focus:outline-none">
                                      <svg
                                        xmlns="http://www.w3.org/2000/svg"
                                        fill="none"
                                        viewBox="0 0 24 24"
                                        strokeWidth="1.5"
                                        stroke="currentColor"
                                        className="w-4 md:w-5 h-4 md:h-5"
                                      >
                                        <path
                                          strokeLinecap="round"
                                          strokeLinejoin="round"
                                          d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10"
                                        />
                                      </svg>
                                    </button>
                                  </Link>
                                </div>
                              </td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                    </div>
                  </div>
                </div>
              </div>
            </section>
          ) : (
            <div className="text-center text-lg text-primary my-20 px-5 font-semibold">
              You have not posted any job yet
            </div>
          )}
        </div>
      )}
    </div>
  );
};

export default MyJobs;
